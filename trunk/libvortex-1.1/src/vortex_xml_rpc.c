					    ...
					    axlPointer           __invocator_data)
{
	...
	if (NODE_CMP_NAME (node, "fault")) {
		...
		if (! __vortex_xml_rpc_get_fault_values (node, &faultCode, &faultString, &value)) {
							    ...
							    axlPointer                user_data)
{
	...
	vortex_thread_pool_new_task ((VortexThreadFunc) __vortex_xml_rpc_invoke, data);
	return;
#endif
}
...
XmlRpcMethodCall * __vortex_xml_rpc_frame_received_parse_method_call (VortexFrame * frame)
{
	...
	while ((param_count > 0) && (iterator < axl_node_get_child_num (node))) {
		...
		value = __vortex_xml_rpc_parse_value (node_aux);
				      ...
				      axlPointer         user_data)
{
	...
	if (ctx->xml_rpc_service_dispatch != NULL) {
		
		/* invoke and get the method response */
		iterator = 0;
		while ((method_response == NULL) && (iterator < axl_list_length (ctx->xml_rpc_service_dispatch))) {
			/* get the dispatch node */
			dispatch_node = axl_list_get_nth (ctx->xml_rpc_service_dispatch, iterator);
 ...
 */
bool     __vortex_xml_rpc_start_msg (char              * profile,
				     int                 channel_num,
				     VortexConnection  * connection,
				     char              * serverName,
				     ...
				     axlPointer          user_data)
{
	...
	if (ctx->xml_rpc_service_dispatch == NULL) {
		...
		iterator      = 0;
		dispatch_node = NULL;
		while (!accept && (iterator < axl_list_length (ctx->xml_rpc_service_dispatch))) {
				  ...
				  XmlRpcMethodResponse * method_response)
{
	 ...
	 * right data */
	v_return_if_fail (method_call);
	v_return_if_fail (method_response);
							    ...
							    
{
	...
	axl_list_add (ctx->xml_rpc_service_dispatch, node);

	/* enable XML-RPC profile */
	vortex_profiles_register (VORTEX_XML_RPC_PROFILE,
				  /* the start handler */
				  NULL, NULL, 
				  /* close handler */
				  NULL, NULL, 
	...
	vortex_profiles_register_extended_start (VORTEX_XML_RPC_PROFILE,
...
bool                vortex_xml_rpc_listener_parse_conf_and_start_listeners ()
{
	...
	return vortex_listener_parse_conf_and_start ();
}
...
/** 
 * @internal Inits the vortex xml-rpc module state.
 * 
...
void vortex_xml_rpc_init    (VortexCtx * ctx)
{
	...
	ctx->xml_rpc_service_dispatch  = axl_list_new (axl_list_always_return_1, axl_free);
#endif
}

/** 
 * @internal Terminates the xml-rpc module state from the provided
 * context.
 * 
...
void vortex_xml_rpc_cleanup (VortexCtx * ctx)
{
	...
	/* free the list */
	axl_list_free (ctx->xml_rpc_service_dispatch);
	ctx->xml_rpc_service_dispatch = NULL;
